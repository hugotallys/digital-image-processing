...
# ╔═╡ 0a88b6f0-a8f1-11ed-398f-531bd6c75324
using Images

# ╔═╡ ec4f7854-1fef-401a-b8b3-6cc9f384da25
md"
...
# ╔═╡ dbb7a81d-e237-4bc8-8701-fa684972a5fa
md"Our images are simply discretizations of real functions $z(x,y) : \mathcal{R}^2 \rightarrow \mathbb{R}$. We begin defining the interval $\mathcal{R} = [-5,5]$ (square with side 5)."

# ╔═╡ bcf9e451-89e3-42d6-b890-5f31b567050f
range = -5:0.01:5

# ╔═╡ c369e77e-0b29-42da-b772-e7b25d82a1f7
md"
### Paraboloid

To obtain a paraboloid with some scaled axis by a factor of $s \geq 0$ we set:

$$z(x,y) = (s \cdot x)^2 + y^2$$

where we choose to scale the x-axis.
"

# ╔═╡ 16502028-82cc-482f-93d9-6186326f3d9c
begin
...
# ╔═╡ 7eebb0e3-04aa-4d3f-83b0-4aeec29bb269
md"
## Rotated Sine
Here we choose the sine function to be $f(x,y) = sin(x)$. We also rotate the points by an arbitrary angle $\theta$ around the $z$ axis (pointing out of screen) by applyig the rotation matrix:

$$R(θ)=\begin{bmatrix}cos(θ) && sin(θ) \\ -sin(θ) && cos(θ)\end{bmatrix}$$
"

# ╔═╡ 2f0909e2-c1e9-4689-b868-54b2e099bf45
begin
...
# ╔═╡ b2eb200e-9d51-4bed-8f0c-88c1849ebb8a
md"
## Gaussian
The gaussian function in its basic form:

$$f(x,y)=exp\left(-\frac{1}{2}(x^2+y^2)\right)$$
"

# ╔═╡ 2195349d-c97b-4823-b92e-17f38013922a
begin
	gauss = [ exp(-0.5*(x^2+y^2)) for x=range, y=range ]
	gauss_img = Gray.(gauss) # no need to normalize
end

# ╔═╡ 4de5b86a-c1ab-4f8e-8b04-88df896c92c4
md"
## Rotated Gaussian
The gaussian function in its most general form:

$$f(x,y)=exp\left(-\frac{(x - \mu x)^2}{2 \sigma x}-\frac{(y - \mu y)^2}{2 \sigma y}\right)$$

We also perform a rotation of $\theta$ radians around $z$.
"

# ╔═╡ 2f5f52e1-58e6-4543-b430-a1f63ccb1a50
begin
	θ = π / 4
	μx, μy = -1.0, 1.0
	σx, σy = 1.25, .75
